    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            for (ConsumerRecord<String, byte[]> record : records) {
                ...
                ExtremeColdWarning warning = gsonCEP.fromJson(string, ExtremeColdWarning.class);
                pushDataToCEPIndex(topic, warning.getAvg(),
                        warning.getEvent1().getTimestamp(),
                        warning.getEvent2().getTimestamp());
                SendEmail sendEmail=new SendEmail();
                sendEmail.send(warning.toString());
            }
        }
    }
}
